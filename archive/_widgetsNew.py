...
class junkCollection():
    ...
    def pullTab(self,gui,phone,gs,fx,desktop,adargs=None):
        ...
        if(gs.ACTIVEWIDGET=='pullTab'):
            ...
            # --------DIMENSIONS
            windowX    = widgetNodeX
            windowY    = widgetNodeY - 50
            winHeight,winWidth = gui.mechBoxMed[0].get_rect().h,gui.mechBoxMed[0].get_rect().w
            cptx      = windowX + 0.37*winWidth
            cpty      = windowY + 0.12*winHeight
            ...
            # draw screen window
            # self.mechBoxMed
            #drawImage(gui.screen, gui.mechBoxMed[0],(windowX,windowY))
            drawSelectableImage(gui.mechBoxMed[0],gui.mechBoxGreen[0],(windowX,windowY),gui)

            LeftXMargin = windowX+0.2*winWidth
            # ----------Heading 
            drawText(gui.screen,gui.nanoNokiaFont, 'Can Pull Tabs',cptx,cpty, colour=(205, 230, 180))
            ...
            if(gs.totalCantabs<gs.cantabLimit):
                ...
                #value incremented 
                gs.cantabs, endX, endY =  gui.incrementableWidget(collX,collY,'Picked up', gs.cantabs,cap=cap,userInput=gui.user_input.returnedKey,incrementKey='c')
                # Sell button
